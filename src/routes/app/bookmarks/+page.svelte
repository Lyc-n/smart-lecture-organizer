	...
	<div class="mx-auto max-w-2xl">
		...
		{#if bookmarks.length === 0}
			<div class="rounded-xl bg-slate-900 light:bg-white border border-slate-800 light:border-slate-200 p-8 text-center">
				...
				<p class="text-slate-400 light:text-slate-500">Belum ada bookmark. Bookmark item atau grup untuk akses cepat.</p>
			</div>
		{:else}
				...
				{#each bookmarks as bm (bm.id)}
					{#if bm.itemId}
						<button
							...
							class="flex w-full items-center gap-3 rounded-xl bg-slate-900 light:bg-white border border-slate-800 light:border-slate-200 p-3 text-left transition hover:border-slate-700 light:hover:border-slate-300"
						>
							...
							<div class="min-w-0 flex-1">
								<div class="truncate text-sm font-medium text-slate-100 light:text-slate-900">{bm.itemName}</div>
								<div class="text-xs text-slate-500 light:text-slate-400">Item &middot; {formatDate(bm.createdAt)}</div>
							</div>
						</button>
					{:else if bm.groupId}
						<button
						...
						>
							...
							<div class="min-w-0 flex-1">
								<div class="truncate text-sm font-medium text-slate-100 light:text-slate-900">{bm.groupName}</div>
								<div class="text-xs text-slate-500 light:text-slate-400">Grup &middot; {formatDate(bm.createdAt)}</div>
							</div>
						</button>
					{/if}
				{/each}
